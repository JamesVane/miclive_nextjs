...
interface CreateSpecificEventProps {
	dateErrorMessage: string;
	setDateErrorMessage: React.Dispatch<React.SetStateAction<string>>;
}

function CreateSpecificEventMobile({
	dateErrorMessage,
	setDateErrorMessage,
}: CreateSpecificEventProps) {
	...
	return (
		<>
			...
			<div className={styles.specific_main_div}>
				<DatePicker
							...
							if (specificEvent.start_time) {
								...
								if (time < startTime) {
									...
									dispatch(
										setSpecificEvent({
											specificKey: "end_time",
											specificValue: adjustedTime.toObject() as any,
										})
									);
								} else {
									dispatch(
				...
				<div className={styles.specific_event_bumper}></div>
			</div>
...
export default CreateSpecificEventMobile;
